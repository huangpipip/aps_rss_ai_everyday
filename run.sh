...
run_in_dir() {
    ...
    if [ -d "$REPO_ROOT/.venv" ]; then
        ...
        uv run --project "$REPO_ROOT" --directory "$workdir" "$@"
    else
        (
            cd "$workdir"
            "$@"
...
# 当前仓库保留本地调试入口，并提供后端版 GitHub Actions 工作流
# This repository keeps a local debug entrypoint and a backend-only GitHub Actions workflow
...
if [ -z "$OPENAI_API_KEY" ]; then
    ...
    echo "   export APS_FEEDS=\"prl=https://feeds.aps.org/rss/recent/prl.xml\""
    ...
    export APS_FEEDS="${APS_FEEDS:-prl=https://feeds.aps.org/rss/recent/prl.xml}"
    export MODEL_NAME="${MODEL_NAME:-gpt-4o-mini}"
    export OPENAI_BASE_URL="${OPENAI_BASE_URL:-https://api.openai.com/v1}"
    
    echo "🔧 当前配置 / Current configuration:"
    echo "   LANGUAGE: $LANGUAGE"
    echo "   APS_FEEDS: $APS_FEEDS"
    echo "   MODEL_NAME: $MODEL_NAME"
    echo "   OPENAI_BASE_URL: $OPENAI_BASE_URL"
fi

export PAPER_LIMIT="${PAPER_LIMIT:-}"
...
today=`date -u "+%Y-%m-%d"`
today_data_file="$REPO_ROOT/data/${today}.jsonl"
today_ai_file="$REPO_ROOT/data/${today}_AI_enhanced_${LANGUAGE}.jsonl"

echo "本地测试：抓取 $today 的 APS RSS 文章... / Local test: Fetching APS RSS entries for $today..."

# 第一步：爬取数据 / Step 1: Crawl data
echo "步骤1：开始抓取 APS RSS... / Step 1: Fetching APS RSS..."
if [ -n "$PAPER_LIMIT" ]; then
    echo "🧪 测试模式：限制抓取文章数为 $PAPER_LIMIT / Test mode: limiting crawl to $PAPER_LIMIT papers"
fi

# 检查今日文件是否已存在，如存在则删除 / Check if today's file exists, delete if found
if [ -f "$today_data_file" ]; then
    echo "🗑️ 发现今日文件已存在，正在删除重新生成... / Found existing today's file, deleting for fresh start..."
    rm "$today_data_file"
    echo "✅ 已删除现有文件：$today_data_file / Deleted existing file: $today_data_file"
else
    echo "📝 今日文件不存在，准备新建... / Today's file doesn't exist, ready to create new one..."
fi

run_in_dir "$REPO_ROOT/daily_arxiv" python daily_arxiv/fetch_aps.py --data ../data/${today}.jsonl

if [ ! -f "$today_data_file" ]; then
    echo "抓取失败，未生成数据文件 / Fetching failed, no data file generated"
    exit 1
fi
...
set +e
run_in_dir "$REPO_ROOT/daily_arxiv" python daily_arxiv/check_stats.py
dedup_exit_code=$?
set -e
...
if [ "$PARTIAL_MODE" = "false" ]; then
    ...
    run_in_dir "$REPO_ROOT/ai" python enhance.py --data ../data/${today}.jsonl
...
if [ "$PARTIAL_MODE" = "false" ] && [ -f "$today_ai_file" ]; then
    echo "📄 使用AI增强后的数据进行转换... / Using AI enhanced data for conversion..."
    run_in_dir "$REPO_ROOT/to_md" python convert.py --data ../data/${today}_AI_enhanced_${LANGUAGE}.jsonl
...
if [ "$PARTIAL_MODE" = "false" ]; then
    ...
    echo "   ✅ APS RSS 抓取 / APS RSS fetching"
