...
class ArxivSpider(scrapy.Spider):
    def __init__(self, *args, **kwargs):
        ...
        paper_limit = os.environ.get("PAPER_LIMIT", "").strip()
        # 保存目标分类列表，用于后续验证
        self.target_categories = set(map(str.strip, categories))
        self.paper_limit = int(paper_limit) if paper_limit else 0
        self.paper_count = 0
        self.start_urls = [
    ...
    def parse(self, response):
        ...
        for paper in response.css("dl dt"):
            if self.paper_limit and self.paper_count >= self.paper_limit:
                self.logger.info(
                    "Reached paper limit %s, stopping crawl",
                    self.paper_limit,
                )
                return
